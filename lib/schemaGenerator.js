...
export function generateData(schema, rowCount = 100, seed = null, locale = 'en') {
  // Using fakerEN_US ensures all data is in English
  if (seed !== null) {
    faker.seed(seed);
  }
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    schema.columns.forEach(column => {
      row[column.name] = generateFieldValue(column, i);
    });
...
function generateFieldValue(column, index) {
  ...
  switch (type) {
    ...
    case 'text':
      return faker.hacker.phrase().split(' ').slice(0, config.wordCount || 3).join(' ');
    ...
    case 'loremParagraph':
      // Generate English paragraph using hacker phrases
      const sentences = [];
      for (let i = 0; i < (config.paragraphs || 3); i++) {
    ...
    case 'loremWords':
      return faker.hacker.phrase().split(' ').slice(0, config.wordCount || 5).join(' ');
    ...
    case 'paragraph':
      const paraSentences = [];
      for (let i = 0; i < 3; i++) {
        paraSentences.push(faker.hacker.phrase());
      }
      return paraSentences.join(' ');
      
    case 'hashtag':
      return '#' + faker.commerce.productAdjective().replace(/\s+/g, '');
    ...
    case 'slug':
      return (faker.hacker.verb() + '-' + faker.hacker.noun() + '-' + faker.hacker.adjective()).toLowerCase().replace(/\s+/g, '-');
